...
#pragma warning restore 0649 // Assigned in Unity inspector
    ...
    {
        return bodyPosition + ColPos;
    }

    Vector3 ColPos => col.center + col.transform.localPosition;
    /// <summary>
    /// Position of the feet (aka bottom) of the body
    /// </summary>
    public Vector3 FootPosition => transform.TransformPoint(ColPos + Vector3.down * col.size.y/2.0f);
    /// <summary>
    ...
    [SerializeField] LayerMask collisionMask = new LayerMask();
    ...
    public void DeferredCollideAndSlide(ref Vector3 bodyPosition, ref Vector3 bodyVelocity, Collider other)
    {
        ...
        Vector3 colliderPos = GetCenterAtBodyPosition(bodyPosition);
    ...
    private void FixedUpdate()
    {
        ...
        var candidates = Overlap(projectedPos, sizeWithSkin / 2, collisionMask, QueryTriggerInteraction.Ignore);
    ...
    {
        Gizmos.DrawLine(transform.position, transform.position + Pen);
        Gizmos.color = Color.cyan;
        Gizmos.matrix = Matrix4x4.TRS(transform.position, transform.rotation, Vector3.one);
        Gizmos.DrawWireCube(ColPos, col.size + Vector3.one * contactOffset);
    }
}
